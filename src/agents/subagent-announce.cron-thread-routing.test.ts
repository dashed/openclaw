...
describe("cron thread routing fix", () => {
  ...
  it("strips stale threadId for completion-mode direct-send on same channel", async () => {
    // The completion-mode direct-send uses completionDirectOrigin (= requesterOrigin)
    // directly, so threadId from requesterOrigin should pass through unmodified.
    // Since requesterOrigin has no threadId, it should be undefined.
    const { runSubagentAnnounceFlow } = await import("./subagent-announce.js");
    sessionStore = {
      "agent:main:subagent:test": {
        sessionId: "child-session-completion-same",
      },
      "agent:main:main": {
        sessionId: "requester-session-completion-same",
        lastChannel: "slack",
        lastTo: "channel:C123",
        lastThreadId: "stale-completion-thread",
      },
    };
    chatHistoryMock.mockResolvedValueOnce({
    ...
    const didAnnounce = await runSubagentAnnounceFlow({
      childSessionKey: "agent:main:subagent:test",
      childRunId: "run-completion-same-channel",
      requesterSessionKey: "agent:main:main",
      requesterDisplayKey: "main",
    ...
    expect(call?.params?.channel).toBe("slack");
    expect(call?.params?.to).toBe("channel:C123");
    // completionDirectOrigin uses requesterOrigin directly — no threadId should leak.
    expect(call?.params?.threadId).toBeUndefined();
  });
});
