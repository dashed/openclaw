...
describe("subagent-announce wiring", () => {
  it("subagent-announce.ts imports resolveAnnounceOrigin from subagent-announce-delivery", () => {
    // Verify that subagent-announce.ts imports resolveAnnounceOrigin from the
    // delivery module (where upstream extracted it).
    const src = fs.readFileSync(
      path.resolve(import.meta.dirname, "subagent-announce.ts"),
      "utf8",
    );
    const hasImport = src.includes('from "./subagent-announce-delivery.js"');
    expect(hasImport).toBe(true);
    expect(src).toContain("resolveAnnounceOrigin");
  });
});
